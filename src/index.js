...
const TOOL_DEFINITIONS = [
  {
    name: 'flow_connect',
    description: 'FIRST STEP: Connect to Chrome (already open and logged in) via CDP and navigate to Google Flow. Call this before any generation tool. Returns connection status.',
    inputSchema: {
      type: 'object',
      properties: {
        headless: { type: 'boolean', description: 'Headless mode. Keep false — Google Flow requires a visible browser.', default: false },
        open_flow: { type: 'boolean', description: 'Auto-navigate to Google Flow after connecting.', default: true },
      },
    },
  },
  {
  ...
  },
  {
    ...
    description: 'Generate an image in Google Flow. Connects to the open Chrome session, fills the prompt, selects model and ratio, clicks Generate, waits for the result, and downloads the image file to disk. Returns the path to the downloaded image. Use auto_confirm=true to actually generate (consumes credits). Use auto_confirm=false to only prepare without generating.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: { type: 'string', description: 'Text description of the image to generate.' },
        model: { type: 'string', description: 'Image model: "Nano Banana 2" (default, fast), "Nano Banana Pro" (quality), "Imagen 4" (premium).', default: 'Nano Banana 2' },
        auto_confirm: { type: 'boolean', description: 'Set true to actually generate and download the image (uses credits). Set false to only prepare without generating.', default: true },
        ratio: { type: 'string', description: 'Image aspect ratio: "1:1", "16:9", "9:16", "4:3", "3:4".', default: '1:1' },
        reference_images: { type: 'array', items: { type: 'string' }, description: 'Optional: local file paths to use as visual references.' },
        project_name: { type: 'string', description: 'Optional: name for the Flow project. Reuses existing project with same name or creates a new one.' },
        campaign: { type: 'string', description: 'Optional: campaign tag to group related generations (e.g., "summer-2026").' },
      },
      required: ['prompt'],
    },
  },
  {
    ...
    description: 'Generate a video in Google Flow. Connects to the open Chrome session, fills the prompt, selects model, ratio and duration, triggers generation, and returns when the video is ready. Returns the path to the downloaded video file.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: { type: 'string', description: 'Text description of the video to generate.' },
        model: { type: 'string', description: 'Video model: "Omni Flash" (default, fast/free), "Veo 3.1 - Fast", "Veo 3.1 - Quality".', default: 'Omni Flash' },
        ratio: { type: 'string', description: 'Video aspect ratio: "16:9" (landscape), "9:16" (portrait/TikTok).', default: '16:9' },
        duration: { type: 'string', description: 'Video duration: "4s", "6s", "8s".', default: '4s' },
        reference_images: { type: 'array', items: { type: 'string' }, description: 'Optional: local file paths to use as visual references.' },
        project_name: { type: 'string', description: 'Optional: name for the Flow project.' },
        campaign: { type: 'string', description: 'Optional: campaign tag to group related generations.' },
      },
      required: ['prompt'],
    },
  },
